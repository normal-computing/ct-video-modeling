...
class CDEFunction(nn.Module):
    def __init__(self, in_channels, base_channels, channel_mults):
        ...
        channels = [in_channels * 3, now_channels]
    ...
    def forward(self, t, z):
        ...
        return self.func(torch.cat([z, e, d], dim=1)).flatten(start_dim=1)
    ...
    def forward(self, z, solver_ts, spline_ts=None):
        ...
        for i, block in enumerate(self.encoder):
            if i < len(self.encoder) - 1:
                z, sc = self.encode(block, z, solver_ts, spline_ts)
                skips.append(sc)
            else:
        ...
        *_, ch, height, width = z.shape
        ...
        self.function.fit_spline(z, spline_ts)

        z = (
            odeint(
                self.function,
                z[:, 0].flatten(start_dim=1),
                solver_ts,
                rtol=self.rtol,
                atol=self.atol,
